...
namespace HotelProjectUI.Controllers
{
    ...
    [Route("GuestController1/[action]/{id?}")]
    public class GuestController1 : Controller
    {
        ...
        public async Task<IActionResult> AddGuest(CreateGuestDto createGuestDto)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:5027/api/Guest", stringContent);
        ...
        public async Task<IActionResult> UpdateGuest(UpdateGuestDto updateGuestDto)
        {
            ...
            var responseMessage = await client.PutAsync("http://localhost:5027/api/Guest/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
